#!/usr/bin/env bash
# setup-labrain.sh (linux) — clones labrain (private repo) if needed and
# persists $LABRAIN_PATH to your shell rc. Idempotent. The vault is ALWAYS
# "$LABRAIN_PATH/labrain-vault" — no separate variable.
#
...
if [ "$persisted" -eq 0 ]; then
  warn "Could not write to a shell rc file. Add this line manually:"
  printf '\n    export LABRAIN_PATH="%s"\n\n' "$LABRAIN_PATH"
else
  info "Open a new shell (or 'source' your rc) so LABRAIN_PATH is available."
fi
